...
void Menu::DrawImgui(Item* item)
{
	...
	case Item::Path:
		...
		else if (isCallback) // コールバック項目の表示
		{
			pCallback->m_CallbackFunc(false, pCallback->m_uValue.cPath);
			if (ImGui::InputText(item->GetName(),  pCallback->m_uValue.cPath, MAX_PATH))
				pCallback->m_CallbackFunc(true, pCallback->m_uValue.cPath);
		}
		break;
	// ボタンの表示
