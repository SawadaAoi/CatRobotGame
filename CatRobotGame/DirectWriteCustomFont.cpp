...
std::shared_ptr<CustomFontCollectionLoader> pFontCollectionLoader = nullptr;
...
public:
	CustomFontFileEnumerator(IDWriteFactory* factory, const std::vector<std::wstring>& fontFilePaths)
		: refCount_(1), factory_(factory), fontFilePaths_(fontFilePaths), currentFileIndex_(-1)
	{
		factory_->AddRef();
	}
...
HRESULT DirectWriteCustomFont::Init(IDXGISwapChain* swapChain)
{
	...
	pFontCollectionLoader = std::make_shared<CustomFontCollectionLoader>();
...
// フォント名を取得する
const std::wstring& DirectWriteCustomFont::GetFontName(int num)
{
	// フォント名のリストが空だった場合
...
HRESULT DirectWriteCustomFont::SetFont(FontData set)
{
	...
	(
		GetFontFileNameWithoutExtension(Setting.font.c_str()).get(),
		fontCollection.Get(),
		Setting.fontWeight,
		Setting.fontStyle,
		Setting.fontStretch,
		Setting.fontSize,
		Setting.localeName.c_str(),
...
//=================================================================================================================================
HRESULT DirectWriteCustomFont::SetFont(std::wstring fontname, DWRITE_FONT_WEIGHT fontWeight, DWRITE_FONT_STYLE fontStyle,
	DWRITE_FONT_STRETCH fontStretch, FLOAT fontSize, std::wstring localeName,
	DWRITE_TEXT_ALIGNMENT textAlignment, D2D1_COLOR_F Color, D2D1_COLOR_F shadowColor, D2D1_POINT_2F shadowOffset)
{
	HRESULT result = S_OK;

	pDWriteFactory->CreateTextFormat(GetFontFileNameWithoutExtension(fontname).get(), fontCollection.Get(), fontWeight, fontStyle, fontStretch, fontSize, localeName.c_str(), &pTextFormat);
	if (FAILED(result)) { return result; }
...
// options：テキストの整形
//====================================
HRESULT DirectWriteCustomFont::DrawString(std::string str, Vector2<float> pos, D2D1_DRAW_TEXT_OPTIONS options, bool shadow)
{
	HRESULT result = S_OK;
...
std::unique_ptr<WCHAR[]> DirectWriteCustomFont::GetFontFileNameWithoutExtension(const std::wstring& filePath)
{
	...
	// 新しいWCHAR配列を作成
	std::unique_ptr<WCHAR[]> fileName(new WCHAR[fileNameWithoutExtension.length() + 1]);

	// 文字列をコピー
	wcscpy_s(fileName.get(), fileNameWithoutExtension.length() + 1, fileNameWithoutExtension.c_str());
