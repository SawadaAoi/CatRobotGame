...
namespace FontList
{
	...
	{
		L"Assets\\Font\\ipaexm.ttf",			// IPA明朝
		L"Assets\\Font\\851letrogo_007.ttf",	// 851レトロゴ
		L"Assets\\Font\\SmartFontUI.otf",		// IPA明朝
	};
}
...
enum FontType
{
	IPAexMin,		// 漢、英、数(IPAex明朝シンプル)
	Letrogo,		// 漢、英、数(大正レトロ)
	SmartFontUI,	// 漢、英、数(SmartFontUI)
};
...
struct FontData
{
	...
	WRL::ComPtr<IDWriteFontCollection> fontCollection;		// フォントコレクション
...
public:
	...
	HRESULT SetFont
	(
		//const WCHAR* fontname,						// フォント名
		std::wstring fontname,						// フォント名
		DWRITE_FONT_WEIGHT		fontWeight,			// フォントの太さ
		DWRITE_FONT_STYLE		fontStyle,			// フォントスタイル
		DWRITE_FONT_STRETCH		fontStretch,		// フォントの幅
		FLOAT					fontSize,			// フォントサイズ
		//const WCHAR* localeName,					// ロケール名
	...
	// pos：描画ポジション
	// options：テキストの整形
	HRESULT DrawString(std::string str, Vector2<float> pos, D2D1_DRAW_TEXT_OPTIONS options, bool shadow = false);
	...
	// フォント名を取得する
	const std::wstring& GetFontName(int num);
	...
	//std::shared_ptr<IDWriteFontCollection> fontCollection;
...
private:
	...
	//std::vector<std::shared_ptr <IDWriteFontFile>> pFontFileList;
	...
	std::unique_ptr<WCHAR[]> GetFontFileNameWithoutExtension(const std::wstring& filePath);
